...
class CIRRDataset(Dataset):
    ...
    def __init__(self, split: str, mode: str, preprocess: callable):
        ...
        """
        self.preprocess = preprocess
        # self.preprocess_384 = preprocess_384
        self.mode = mode
        self.split = split
        ...
        try:
            if self.mode == 'relative':
                ...
                if self.split == 'train':
                    ...
                    reference_image_path = base_path / 'cirr_datasets' / self.name_to_relpath[reference_name]
                    reference_image = self.preprocess(PIL.Image.open(reference_image_path))
                    return pair_id, reference_name, reference_image, rel_caption, group_members
...
class CIRRDataset_val_submission(Dataset):
    """
       CIRR dataset class which manage CIRR data
       The dataset can be used in 'relative' or 'classic' mode:
           - In 'classic' mode the dataset yield tuples made of (image_name, image)
        ...
        try:
            if self.mode == 'relative':
                ...
                if self.split == 'train':
                    ...
                    target_hard_name = self.triplets[index]['target_hard']
                    # return reference_name, target_hard_name, rel_caption, group_members
                    return pair_id, reference_name, reference_image, rel_caption, group_members, target_hard_name
...
class CIRRDataset_hinge(Dataset):
    """
       CIRR dataset class which manage CIRR data
       The dataset can be used in 'relative' or 'classic' mode:
           - In 'classic' mode the dataset yield tuples made of (image_name, image)
        ...
        try:
            if self.mode == 'relative':
                ...
                if self.split == 'train':
                    ...
                    target_image = self.preprocess(PIL.Image.open(target_image_path))
                    # return reference_name, target_hard_name, rel_caption, group_members
                    return reference_image, reference_name, target_hard_name, rel_caption, group_members, target_image
